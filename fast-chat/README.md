...
- 🎭 **Auto-generated nicknames** - Docker-style naming (adjective_scientist)
- 📝 **Message logging** - Async JSONL logging to disk
- 🏗️ **Multi-language ready** - Organized structure for Go/Rust implementations
...
```
JOIN                     - Join the chat (server assigns nickname)
SEND|{text}              - Send a message
HISTORY|{count}          - Request history
PING                     - Keep-alive
```

**Note**: Nicknames are automatically generated by the server using Docker-style naming (e.g., `clever_einstein`, `happy_turing`, `brave_curie`).
...
<div data-target="#messages" data-action="append">
  <div class="msg" data-id="1234567890123">
    <span class="nick">clever_einstein</span>
...
│   │   │   ├── nicknames.go    # Docker-style nickname generator
│   │   │   └── logger.go       # Async message logger (JSONL)
│   │   └── handlers/
...
2. Users are automatically assigned Docker-style nicknames
3. Send messages and observe:
   - Real-time message delivery
   - Latency measurements (green text)
   - Join/leave notifications with generated nicknames
   - User count updates
...
2025-10-11 16:50:02 User clever_einstein joined (conn: 123e4567-e89b-12d3-a456-426614174000)
2025-10-11 16:50:03 Message from clever_einstein: Hello!
```

### Message Logging

All messages are logged asynchronously to `logs/messages.jsonl` in JSON Lines format:
```bash
# View logged messages
tail -f logs/messages.jsonl

# Count messages
wc -l logs/messages.jsonl
```

Each line contains a complete message with timestamp, user, and content.
